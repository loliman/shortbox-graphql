...
export const resolvers = {
  Query: {
    export: async (_, { filter, type }, context) => {
      ...
      await asyncForEach(res[0] as any[], async (r: any) => {
        let publisher = {
      ...
      if (type === 'txt') {
        return JSON.stringify(
          (await convertFilterToTxt(filter, loggedIn)) + (await resultsToTxt(sortedResponse)),
        );
      } else if (type === 'csv') {
...
async function convertFilterToTxt(filter, loggedIn) {
  ...
  if (filter.releasedates) {
    ...
    filter.releasedates.forEach(
      (r) => (s += dateFormat(new Date(r.date), 'dd.mm.yyyy') + ' ' + r.compare + ', '),
    );
    s = s.substr(0, s.length - 2) + '\n';
  }
  ...
  if (filter.onlyOnePrint) s += '\t\tNur einfach auf deutsch erschienen\n';
  if (loggedIn && filter.onlyCollected) s += '\t\tIn Sammlung enthalten\n';
  if (loggedIn && filter.onlyNotCollected) s += '\t\tNicht in Sammlung enthalten\n';
  if (filter.noPrint) s += '\t\tNicht auf deutsch erschienen\n';
  ...
  if (filter.numbers) {
    s += '\t\tNummer: ';
    filter.numbers.forEach(
      (n) =>
        (s += n.number + ' ' + n.compare + (n.variant !== '' ? ' [' + n.variant + ']' : '') + ', '),
    );
    s = s.substr(0, s.length - 2) + '\n';
  }
  ...
  if (
    ...
    !filter.onlyOnePrint &&
    loggedIn &&
    !filter.onlyCollected &&
    !filter.onlyNotCollected
  )
    s += '\t\t-\n';
  ...
  if (!filter.individuals) s += '\t\t-\n';

  s += '\tAuftritte\n';

  if (filter.appearances) {
    filter.appearances.forEach(
      (i) =>
        (s += '\t\t' + i.name.replace(new RegExp('"', 'g'), "'") + ' (' + getType(i.type) + ')\n'),
    );
  }

  if (!filter.appearances) s += '\t\t-\n';
...
export function createFilterQuery(
  ...
  let columns =
    'p.name as publishername, p.id as publisherid, ' +
  ...
  if (print && !overview) groupby = 'i.number, s.title, s.volume, p.name';
  else if (selected.publisher && !overview) {
    groupby = 'i.number';
    where =
      " and s.title = '" +
      escapeSqlString(selected.title) +
      "' and s.volume = " +
      selected.volume +
      " and p.name = '" +
      escapeSqlString(selected.publisher.name) +
      "' ";
  } else if (selected.name && !overview) {
    groupby = 's.title, s.volume';
    where = " and p.name = '" + escapeSqlString(selected.name) + "' ";
  } else if (!overview) groupby = 'p.name';
  ...
  if (filter.releasedates && filter.releasedates.length > 0) {
    filter.releasedates.map(
      (releasedate) =>
        (where +=
          " and '" +
          dateFormat(new Date(releasedate.date), 'yyyy-mm-dd') +
          "' " +
          releasedate.compare +
          ' i.releasedate '),
    );
  ...
  if (!overview) rawQuery += ' group by ' + groupby;
  ...
  if (filter.appearances && filter.appearances.length > 0) {
    ...
    for (let i = 2; i > 0; i--) {
      intersect +=
        ...
        ' left join ' +
        type +
        ' st on i.id = st.fk_issue ' +
        ' left join story_appearance stapp on ' +
        (i === 2 ? 'st.id' : 'st.fk_parent') +
        ' = stapp.fk_story ' +
        ' left join appearance app on stapp.fk_appearance = app.id ' +
        ' where i.id is not null ' +
        " and ((app.name like '%" +
        escapeSqlString(filter.appearances) +
        "%'))";

      if (i === 2) intersect += ') or i.id in (';
    }
  ...
  if (filter.individuals && filter.individuals.length > 0) {
    ...
    for (let i = 2; i > 0; i--) {
      ...
      filter.individuals.map((individual, i) => {
        ...
        intersect +=
          " (indi.name = '" +
          escapeSqlString(individual.name) +
          "' and stindi.type = '" +
          individual.type +
          "')";
      });
  ...
  if (filter.arcs && filter.arcs.length > 0) {
    ...
    intersect += us
      ? ' left join issue_arc ia on i.id = ia.fk_issue '
    ...
    intersect += ' left join arc a on ia.fk_arc = a.id ' + ' where i.id is not null ' + ' and (';
  ...
  if (loggedIn && filter.onlyCollected && !filter.us) {
    intersect +=
      ' ' +
      unionOrIntersect(filter.and) +
      '  (select i.id from issue i group by i.fk_series, i.number having max(i.collected) = 1) ';
  }
  ...
  if (loggedIn && filter.onlyNotCollected && !filter.us) {
    intersect +=
      ' ' +
      unionOrIntersect(filter.and) +
      '  (select i.id from issue i group by i.fk_series, i.number having max(i.collected) = 0 or max(i.collected) is null) ';
  }
  ...
  if (filter.publishers && filter.publishers.filter((p) => p.us !== filter.us).length > 0) {
    intersect +=
      ...
      ' left join issue i on s.id = i.fk_series ' +
      ' left join story st on i.id = st.fk_issue ' +
      ' left join story stjoin on ' +
      (us ? 'st.id =' : 'st.fk_parent =') +
      (us ? 'stjoin.fk_parent ' : 'stjoin.id ') +
      ' left join issue ijoin on stjoin.fk_issue = ijoin.id ' +
  ...
  if (filter.series && filter.series.filter((p) => p.publisher.us !== filter.us).length > 0) {
    ...
    filter.series
      .filter((p) => p.publisher.us !== filter.us)
      .map((series, i) => {
        if (i > 0) intersect += ' OR ';
        intersect +=
          " (sjoin.title = '" +
          escapeSqlString(series.title) +
          "' and sjoin.volume = " +
          series.volume +
          " and pjoin.name = '" +
          escapeSqlString(series.publisher.name) +
          "')";
      });
  ...
  if ((filter.reprint || filter.exclusive) && !us) {
    intersectContains +=
      ...
      ' left join ' +
      type +
      ' stjoin ON st.fk_parent = stjoin.id' +
      ' where i.id is not null ' +
  ...
  if (filter.onlyTb && us) {
    intersectContains +=
      ...
      ' left join ' +
      type +
      ' stjoin ON st.id = stjoin.fk_parent' +
      ' left join issue ijoin on stjoin.fk_issue = ijoin.id ' +
      ' where i.id is not null ' +
  ...
  if (intersectContains !== '') intersect += intersectContains; //" and i.id " + (filter.reprint ? "not in" : "in") + " (" + intersectContains + ")";

  if (intersect !== '') intersect = ' and (' + intersect.substring(6) + ')';
  ...
  if (order) {
    ...
    if (!selected) {
      rawQuery +=
        ' ORDER BY p.name, s.title, s.volume, ' + fromRoman + ', cast(i.number as unsigned) DESC';
    } else if (selected.name) {
      rawQuery +=
        ' ORDER BY s.title, s.volume, p.name, ' + fromRoman + ', cast(i.number as unsigned) DESC';
    } else if (selected.title) {
      rawQuery +=
        ' ORDER BY ' + fromRoman + ', cast(i.number as unsigned), s.title, s.volume, p.name DESC';
    }
  }

  if (!print && offset !== undefined) rawQuery += ' LIMIT ' + offset + ', 50';
