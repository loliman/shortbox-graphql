  ...
  try {
    ...
    await asyncForEach(input, async (line) => {
      ...
      let filter = {
        ...
        '$Series.title$': {
          [Op.like]: '%' + issue.series.title.replace(/[\W_]+/g, ' ').replace(/\s/g, '%') + '%',
        },
        number: issue.number,
      };

      if (issue.series.startyear) filter['$Series.startyear$'] = issue.series.startyear;
      ...
      if (issues.length > 1) {
        let defaultIssue = issues.filter(
          (i) =>
            (i.format === 'Softcover' ||
              i.format === 'Heft' ||
              i.format === 'Album' ||
              i.format === 'Taschenbuch' ||
              i.format === 'Magazin') &&
            i.variant === '',
        );

        if (defaultIssue.length === 0) {
          defaultIssue = issues.filter(
            (i) => i.variant.toLowerCase().indexOf('kiosk') > -1 || i.variant === '',
          );
        }
