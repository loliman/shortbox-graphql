...
let BASE_URL =
  '/Users/christian/Downloads/cmro.travis-starnes.com/character_details.php-character=814';
var stream;
...
async function crawlReadingOrder() {
  try {
    ...
    stories = stories.map((row) => {
      ...
      if (title.startsWith('The')) title = title.substring(3).trim();
    ...
    await asyncForEach(stories, async (story) => {
      ...
      if (issues.length === 0) {
        ...
        let children = await models.Story.findAll({
