...
async function fixSeriesYear() {
  ...
  await asyncForEach(series, async (series, i, a) => {
    let firstYear = await models.Issue.findOne({
      attributes: [
        [models.sequelize.fn('MIN', models.sequelize.col('Issue.releasedate')), 'releasedate'],
      ],
      where: {
        fk_series: series.id,
      },
    ...
    let endYear = await models.Issue.findOne({
      attributes: [
        [models.sequelize.fn('MAX', models.sequelize.col('Issue.releasedate')), 'releasedate'],
      ],
      where: {
        fk_series: series.id,
      },
    ...
    if (endYear.releasedate !== null) {
      ...
      if (endYear >= new Date().getFullYear() - 1) endYear = 0;
    }

    if (firstYear !== null && endYear !== null) {
      console.log(
        '[' + i + '/' + a.length + '] Fixing ' + series.title + ' (Vol. ' + series.volume + ')',
      );
