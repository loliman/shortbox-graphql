...
boot(async () => {
  ...
  try {
    ...
    await asyncForEach(input, async (line) => await ls(line));
  } catch (e) {
...
async function ls(line) {
  if (line.trim() === '' || line.split(';').length !== 3) return;
  ...
  await asyncForEach(issues, async (issue) => {
    ...
    let title =
      series.title + ' (' + series.startyear + ') #' + parent.number + ' [' + publisher.name + ']';

    writeLine(title);
    if (parent.title !== '') writeLine('Titel: ' + parent.title);
    ...
    await asyncForEach(v, async (v) => {
      ...
      if (v.isbn && v.isbn > 0) writeLine('' + 'ISBN: ' + v.isbn + '');
