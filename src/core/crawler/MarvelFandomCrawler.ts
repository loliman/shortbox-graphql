...
export class MarvelFandomCrawler implements Crawler {
  ...
  private static INDEX_URI: string = MarvelFandomCrawler.BASE_URI + '/index.php';
  ...
  private async finalizeStories(issue: Issue) {
    await MarvelFandomCrawler.asyncForEach(issue.stories, async (story: Story, idx: number) => {
      story.number = idx + 1;
    ...
    issue.variants.forEach((v, i) => {
      ...
      for (let j = i + 1; j < issue.variants.length; j++) {
        if (issue.variants[j].variant === v.variant) {
          issue.variants[j].variant += ' ' + this.getFromAlphabet(duplicateCount + 1);
          duplicateCount++;
        }
      }
  ...
  private async crawlCovers(issue: Issue) {
    ...
    await MarvelFandomCrawler.asyncForEach(issue.variants, async (issue: Issue) => {
      await this.crawlCover(issue.cover, issue);
    });
  }
  ...
  private async crawlInfobox(issue: Issue) {
    ...
    if (!$.parse) {
      throw new Error('[CRAWLER] ' + MarvelFandomCrawler.generateIssueName(issue) + ' - Not found');
    }

    $ = $.parse.wikitext['*'].split('\n');

    if ($[0].trim().startsWith('#REDIRECT')) {
      console.log(MarvelFandomCrawler.generateIssueName(issue) + ' - ' + $[0].trim());
      ...
      issue.series.volume = Number.parseInt(redirect.substr(0, redirect.indexOf(' ')).trim());
  ...
  private crawlInfoboxLine(line: string, indexOfLine: number, issue: Issue, $: any) {
    ...
    if (type.indexOf('Artist') !== -1) {
      ...
      let month: number = isNaN(Number.parseInt(content))
        ? 'JanFebMarAprMayJunJulAugSepOctNovDec'.indexOf(content.substr(0, 3)) / 3 + 1
        : Number.parseInt(content);

      month = Math.ceil(month);
      month = month < 1 || month > 12 ? 1 : month;
      issue.releasedate = issue.releasedate.substr(0, 5) + month + issue.releasedate.substr(7);
    } else if (type.startsWith('Year')) {
      issue.releasedate = Number.parseInt(content) + issue.releasedate.substr(4);
  ...
  private extractAppearances(count: number, issue: Issue, indexOfLine: number, $: any) {
    //Sooooometimes (again...) there are no individuals defined, so we have to create the story during the
    //appearing block :(
    count -= 1;
    if (!issue.stories[count]) MarvelFandomCrawler.createStory(count + 1, issue);
    ...
    while (!currentLine.startsWith('|') && currentLine.indexOf('=') === -1) {
      ...
      if (currentLine.startsWith("'''")) {
        ...
        apps.forEach((app: string) => {
          ...
          if (app.indexOf('<small>') > -1) app = app.substr(0, app.indexOf('<'));

          if (app.startsWith('"') && app.endsWith('"')) app = app.substr(1, app.length - 1);
  ...
  private static extractReprint(content: string, count: number, type: string, issue: Issue) {
  ...
  private extractAdaptedFrom(content: string, count: number, issue: Issue) {
    MarvelFandomCrawler.createStory(count, issue);
    this.addIndividual(content, 'ORIGINAL', issue.stories[count - 1].individuals);
  }

  private extractColourist(content: string, count: number, issue: Issue) {
    MarvelFandomCrawler.createStory(count, issue);
    this.addIndividual(content, 'COLORIST', issue.stories[count - 1].individuals);
  }

  private extractLetterer(content: string, count: number, issue: Issue) {
    MarvelFandomCrawler.createStory(count, issue);
    this.addIndividual(content, 'LETTERER', issue.stories[count - 1].individuals);
  }
  ...
  private extractPenciler(content: string, count: number, issue: Issue) {
    MarvelFandomCrawler.createStory(count, issue);
    this.addIndividual(content, 'PENCILER', issue.stories[count - 1].individuals);
  }

  private extractWriter(content: string, count: number, issue: Issue) {
    MarvelFandomCrawler.createStory(count, issue);
    if (!issue.stories[count - 1].reprintOf)
      this.addIndividual(content, 'WRITER', issue.stories[count - 1].individuals);
  }
  ...
  private extractEditor(content: string, count: number, issue: Issue) {
    MarvelFandomCrawler.createStory(count, issue);
    if (!issue.stories[count - 1].reprintOf)
      this.addIndividual(content, 'EDITOR', issue.stories[count - 1].individuals);
  }

  private static extractVariant(type: string, content: string, count: number, issue: Issue) {
    if (count === 1) {
      return;
    }
  ...
  private static extractGenre($: any, issue: Issue) {
    ...
    if (genre.length > 0) issue.series.genre = $(genre.get(0).nextSibling).text().trim();
  }

  private static extractPublisher($: any, issue: Issue) {
    let publisher = $("span:contains('Publisher: ')");
    if (publisher.length > 0) {
      issue.series.publisher.name = $(publisher.get(0).nextSibling).text().trim();
    } else {
  ...
  private getAppearances(story: Story, currentType: string, name: string, firstApp: boolean) {
    let role = '';
  ...
  private static generateIssueName(issue: Issue) {
    return MarvelFandomCrawler.generateSeriesName(issue.series) + ' ' + issue.number.trim();
  }
  ...
  private removeBraces(string: string): string {
    if (string.endsWith(')'))
      return this.removeBraces(string.substring(0, string.lastIndexOf('(')).trim());
