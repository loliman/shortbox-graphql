...
import {Network, NETWORK_ERROR} from "./network";
...
export class Crawler {
    ...
    async crawlSeries(series) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                response.crawledSeries = await this.crawlSeriesHtml(response.response.html, series);
    ...
    async crawlSeriesHtml(html, series) {
        return new Promise(async (resolve, reject) => {
            try {
    ...
    async crawlIssue(issue) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                response.crawledIssue = await this.crawlIssueHtml(response.response.html, issue);
    ...
    async crawlIssueHtml(html, issue) {
        return new Promise(async (resolve, reject) => {
            try {
                let $ = cheerio.load(html);
                ...
                headers.each((i, e) => {
                    ...
                    while(true) {
                        ...
                        if(next) {
                            ...
                            if(next.is('div') && next.attr('id').indexOf('messageBox') !== -1) {
                                let reprintOf = {series: {}};

                                let reprintOfAsString = $(next).children().first().children('a').first().attr('href');
                                if(reprintOfAsString) {
                                    reprintOfAsString = reprintOfAsString.replace("/wiki/", "");

                                    let splitted = reprintOfAsString.split("_Vol_");

                                    reprintOf.series.title = splitted[0];
                                    while(reprintOf.series.title.indexOf("_") !== -1) {
                                        reprintOf.series.title = reprintOf.series.title.replace("_", " ");
                                    }

                                    reprintOf.series.volume = parseInt(splitted[1].substring(0, splitted[1].indexOf("_")));

                                    reprintOf.number = splitted[1].substring(splitted[1].indexOf("_") + 1);
                                    while(reprintOf.number.indexOf("_") !== -1) {
                                        reprintOf.number = reprintOf.number.replace("_", " ");
                                    }

                                    currentStory.reprintOf = reprintOf;
                                    currentStory.individuals = undefined;
                                }
                            }

                            if(next.is('p')) {
                                if(currentStory.reprintOf)
                                    break;
                                ...
                                if(next.is('ul'))
                                    this.crawlApps(next, $, currentType, currentSubType, currentStory)
                            }
                    ...
                    if($(e).text() === "See Also") {
                        ...
                        if(list) {
                            list.children().each((i, e) => {
                                if($(e).text().indexOf("Reprint") !== -1) {
                                    reprintedPage = "https://marvel.fandom.com" + $(e).children().first().attr("href");
                                }
                            })
                        }
                    }
                ...
                if(reprintedPage)
                    await this.crawlReprints(reprintedPage, res, issue);
    ...
    crawlApps(e, $, currentType, currentSubType, currentStory) {
        ...
        l.each((i, e) => {
            ...
            if($(e).children('ul').length === 0) {
                ...
                ul.each((i, e) => {
                    this.crawlApps(e, $, currentType, currentSubType, currentStory);
                });
            }
        });
    }

    async crawlReprints(url, res, issue) {
        return new Promise(async (resolve, reject) => {
            try {
                let response = await this.network.fetch(url);
                let $ = cheerio.load(response.response.html);

                let issues = $(".category-page__member");

                issues.each((i, e) => {
                    let reprintedIn = {series: {}};

                    let reprintedInAsString = $(e).children("a").first().attr("href");
                    reprintedInAsString = reprintedInAsString.replace("/wiki/", "");

                    let splitted = reprintedInAsString.split("_Vol_");

                    reprintedIn.series.title = splitted[0];
                    while(reprintedIn.series.title.indexOf("_") !== -1) {
                        reprintedIn.series.title = reprintedIn.series.title.replace("_", " ");
                    }

                    if(splitted[1]) {
                        reprintedIn.series.volume = parseInt(splitted[1].substring(0, splitted[1].indexOf("_")));

                        reprintedIn.number = splitted[1].substring(splitted[1].indexOf("_") + 1);
                        while(reprintedIn.number.indexOf("_") !== -1) {
                            reprintedIn.number = reprintedIn.number.replace("_", " ");
                        }
                    } else {
                        reprintedIn.series.volume = 1;
                        reprintedIn.number = "1";
                    }

                    if(!res.reprintedIn)
                        res.reprintedIn = [];

                    res.reprintedIn.push(reprintedIn);
                });
