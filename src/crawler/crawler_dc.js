    ...
    if ($[0].trim().startsWith('#REDIRECT')) {
        ...
        $.forEach((line, i) => {
            ...
            if (line.startsWith('|') && line.indexOf('=') > -1) {
                crawlInfoboxLine(line
                    .replace("|", "").trim(), i, issue, $);
            }
        });
    }
}
...
function extractDates($, issue) {
    ...
    if (publicationDate.length > 0) {
        let text = $(publicationDate.get(0).nextSibling.nextSibling)
            .text()
            .trim();
...
function extractPublisher($, issue) {
    ...
    if (publisher.length > 0) {
        issue.series.publisher.name = $(publisher.get(0).nextSibling.nextSibling)
            .text()
            .trim();
    } else {
