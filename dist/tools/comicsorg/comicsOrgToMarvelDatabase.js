...
async function crawlComicsOrg(idToCrawl) {
    try {
        let issue = await models_1.default.Issue.findOne({ where: { id: idToCrawl } });
        let stories = (await models_1.default.Story.findAll({
            where: { issue_id: issue.id },
            order: [['sequence_number', 'DESC']],
        }))
            .filter((s) => s.type_id === 13 || s.type_id === 19 || s.type_id === 21)
            .reverse();
        await (0, util_1.asyncForEach)(stories, async (story, i) => {
            ...
            let individuals = await models_1.default.Individual.findAll({
                include: [
                    ...
                    {
                        model: models_1.default.IndividualType,
                    },
                ],
                where: {
