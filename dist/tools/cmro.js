...
(0, boot_1.boot)(async () => {
    try {
        ...
        stream = fs_1.default.createWriteStream('reading_order', { flags: 'a' });
...
async function crawlReadingOrder() {
    try {
        let $ = cheerio_1.default.load(fs_1.default.readFileSync(BASE_URL + '&page=1.html'));
        let pagination = $('.pagination');
        let buttons = pagination.get(0).children;
        let lastPage = parseInt(buttons[buttons.length - 2].children[0].data);
        let stories = [];
        for (let i = 1; i <= lastPage; i++) {
            $ = cheerio_1.default.load(fs_1.default.readFileSync(BASE_URL + '&page=' + i + '.html'));
        ...
        await (0, util_1.asyncForEach)(stories, async (story) => {
            console.log('Searching for: ' +
                story.issue.series.title.replace(/[^a-z0-9]/gi, '%').trim() +
                ' (' +
                story.issue.series.startyear +
                ') #' +
                story.issue.number +
                ' [' +
                story.line +
            ...
            let where = {
                ...
                '$Series.title$': {
                    [Op.like]: story.issue.series.title.replace(/[^a-z0-9]/gi, '%').trim(),
                },
            };
            ...
            if (issues.length === 0) {
                writeLine(story.issue.series.title +
                    ' (' +
                    story.issue.series.startyear +
                    ') #' +
                    story.issue.number +
                ...
                let children = await models_1.default.Story.findAll({
                    where: { fk_parent: stories[story.number - 1].id },
                });
                let germanIssues = [];
                await (0, util_1.asyncForEach)(children, async (child) => germanIssues.push(await child.getIssue()));
                writeLine(story.issue.series.title +
                    ' (' +
                    story.issue.series.startyear +
                    ') #' +
                    story.issue.number +
                    ' found in: ');
                await (0, util_1.asyncForEach)(germanIssues, async (issue) => {
